...
reveals it word-by-word with paragraph-aware pacing, holds the final block,
blanks, and loops the reveal forever (a kiosk has no refresh button).
"""
...
END_HOLD = 4.0                # seconds the final block stays on screen
LOOP_PAUSE = 5.0              # seconds of black between reveal loops
...
def reveal_message(stdscr, message: str) -> None:
    """
    Clear the screen and reveal the message word-by-word with WORD_DELAY
    between words. Paragraph breaks become PARAGRAPH_PAUSE blank-line pauses.
    After the final word, hold for END_HOLD, blank for LOOP_PAUSE, then
    restart from the top — forever. The kiosk has no refresh button, so we
    can't end on a frozen frame.
    """
    curses.curs_set(0)
    stdscr.erase()
    stdscr.refresh()
    ...
    while True:
        # Re-read terminal size each loop in case the tty got resized.
        h, w = stdscr.getmaxyx()
        ...
        # Blank the screen and pause before looping back to the top.
        stdscr.erase()
        stdscr.refresh()
